...
def train(args):
    ...
    # Convert diffusion config to a dictionary for manipulation
    diffusion_config = args.diffusion
    
    # Calculate total steps for the flow matching stage if enabled
    flow_matching_steps = 0
    if diffusion_config.get('two_stage', False):
        # Pop the config keys so they aren't passed to MeanFlow directly
        flow_matching_epochs = diffusion_config.pop('flow_matching_epochs', 0)
        diffusion_config.pop('two_stage') # Not needed by MeanFlow constructor
        
        steps_per_epoch = len(train_loader)
        flow_matching_steps = flow_matching_epochs * steps_per_epoch
        if local_rank == 0:
            logger.info(f"MeanFlow initialized for two-stage training. Stage 1 (Flow Matching) will run for {flow_matching_steps} steps.")
    ...
    diffusion = MeanFlow(model, flow_matching_total_steps=flow_matching_steps, **diffusion_config)
