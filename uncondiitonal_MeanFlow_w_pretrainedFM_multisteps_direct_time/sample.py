...
def sample(args):
    ...
    if local_rank == 0:
        gen_dir = os.path.join(args.save_dir, f"EMAgenerated_ep{epoch_checkpoint}_meanflow")
    ...
    for batch in range(batches):
        with torch.no_grad():
            ...
            with torch.no_grad():
                x_gen = diffusion.sample(noise, args.num_steps)
        dist.barrier()
        ...
        if local_rank == 0:
            ...
            save_image(grid, png_path, nrow=20, normalize=True, value_range=(-1, 1))
