...
class DDPM():
    ...
    def loss(self, x, class_labels=None):
        ...
        loss = (noise - self.model(x_noised, t/self.T, class_labels))**2 
        return loss.mean()

    def sample(self, latents, class_labels=None, no_tqdm=False):
        size = latents.shape
        for i in tqdm(range(self.T, 0, -1), disable=no_tqdm):
            ...
            eps, _ = self.pred_eps_(latents, t, class_labels, self.alphas_bar[i])
    ...
    def pred_eps_(self, x, t, class_labels, alpha, clip_x=True):
        ...
        eps = self.model(x, t, class_labels).float()
